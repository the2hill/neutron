...
NOOP_DRIVER_KLASS = ('neutron.services.loadbalancer.drivers.logging_noop.'
        ...
        with self.subnet() as subnet:
            with self.loadbalancer(subnet=subnet) as loadbalancer:
                ...
                with self.listener(loadbalancer_id=lb_id):
                    ...
                    self.assertRaises(loadbalancerv2.LoadBalancerInUse,
        ...
        with self.healthmonitor() as hm:
            ...
            with self.pool(healthmonitor_id=hm_id):
                ...
                pool_data = {'pool': {'tenant_id': self._tenant_id,
                                      ...
                                      'lb_algorithm': 'ROUND_ROBIN',
                                      'admin_state_up': True}}
                self.assertRaises(loadbalancerv2.HealthMonitorInUse,
    ...
    def test_list_members_with_sort_emulated(self):
        with self.pool() as pool:
            ...
            with self.member(pool_id=pool_id, protocol_port=81) as m1:
                with self.member(pool_id=pool_id, protocol_port=82) as m2:
                    with self.member(pool_id=pool_id, protocol_port=83) as m3:
